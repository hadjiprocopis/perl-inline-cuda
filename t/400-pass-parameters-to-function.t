...
my $result = do_saxpy($N, $alpha, \@x, \@y);
ok(defined($result), "do_saxpy() : got back result.");
ok(ref($result) eq 'ARRAY', "do_saxpy() : result is arrayref.");
ok(scalar(@$result) == $N, "do_saxpy() : result has $N items.");
...
for($i=0;$i<$N;$i++){
	...
	$total_error += $error;
	ok($error<$ERROR_TOLERANCE, "do_saxpy() : result is within error bounds (got=$got, expected=$expected).");
}
ok($total_error<$ERROR_TOLERANCE, "do_saxpy() : total error is within bounds ($total_error < $ERROR_TOLERANCE).");
