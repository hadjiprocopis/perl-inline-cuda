...
# we are using bind() to have multiple code blocks one after the other,
# no failures though (unless you want to use eval and catch the dies)
# using the same function name does not seem to be a problem
# and the function code is modified every time.
...
$code =~ s/define N 10/define N 20/;
  Inline->bind( CUDA => $code,
	BUILD_NOISY => 1,
	clean_after_build => 0,
	warnings => 10,
...
#### Now let's modify the source code and see if the change is coming back for calling it another time...
$code =~ s/called/called again!!!!/g;
$code =~ s/define N 20/define N 30/;
  Inline->bind( CUDA => $code,
	BUILD_NOISY => 1,
	clean_after_build => 0,
	warnings => 10,
