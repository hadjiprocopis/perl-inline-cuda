...
# this tries to run Inline in eval-block just for the sake of it, do not use in production

# WARNING: these tests are run under eval-block which is not recommended
...
$retcode = eval {
  require Inline;
  Inline->import( CUDA => Config =>
	BUILD_NOISY => 1,
	clean_after_build => 0,
	warnings => 10,
  );
  Inline->import( CUDA => $code );
  1;
};
is($retcode, 1, "returned good result.") or BAIL_OUT("failed: did not come out of eval with 1 but $retcode");
...
ok(! defined $retcode, "returned failure as expected.") or BAIL_OUT("failed to fail and returned $retcode.");

# AFTER A FAILURE (above) IT DOES NOT WORK ANY MORE!
=pod
