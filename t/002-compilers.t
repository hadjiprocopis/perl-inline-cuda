...
my $ret = Inline::CUDA::Utils::test_compilers($conf);
...
3) This is usually the most common cause of failures:
   ...
   in an obscure, public dir (e.g. /usr/local/gcc84) AND THEN LET
   ...
   Most likely these (change the path to your /usr/local/gcc84/bin/...):
     cc=/usr/bin/cc
     cxx=/usr/bin/c++
     ld=/usr/bin/c++

   Again, you should change /usr/bin/ to /usr/local/gcc84/bin
   (for example, '/usr/local/gcc84' was an example)
   AND ALSO MAKE SURE that /usr/local/gcc84/bin/cc does exist
   or change its name to /usr/local/gcc84/bin/gcc
   or make a link /usr/local/gcc84/bin/cc -> /usr/local/gcc84/bin/gcc
   ...
   CC=/usr/local/gcc84/bin/gcc \
   CXX=/usr/local/gcc84/bin/g++ \
   LD=/usr/local/gcc84/bin/g++ \
   perl Makefile.PL
