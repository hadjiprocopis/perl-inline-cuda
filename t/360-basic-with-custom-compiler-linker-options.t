...
# We are testing failure when passing bogus CFLAGS
# we are catching the failure in an eval because
# Inline::C dies on compilation failure (because of the bogus CFLAGS)
...
die "it should have failed" unless $retcode == 1;
} catch {
	$failed=$_;
};

ok(defined($failed), "${failed}\n\nfailed AS EXPECTED, see above.");
