...
# tests that system-wide configuration is read and is good
# and that a bogus user-specified one is loaded and fails as expected.
# and also if bogus CXX option is specified it fails too
...
### this must fail because configuration file is not found
$failed = undef;
try {
  use Inline;
  Inline->bind( CUDA => $code,
	...
	CONFIGURATION_FILE => File::Spec->catdir($FindBin::RealBin, 't-data', 'Inline-CUDA.fail.confxxx'),
  );
} catch { $failed = $_ };
ok(defined $failed, "failed as expected because of bogus configuration file.") or BAIL_OUT("failed to fail");
...
ok(defined $failed, "failed as expected because of bogus configuration (cc was set manually).") or BAIL_OUT("failed to fail");
