#ifndef CPU_MATRIX_OPS_INT_C
#define CPU_MATRIX_OPS_INT_C

/* our $VERSION = 0.05; */

#include <inlinecuda/utils.h>
#include <inlinecuda/cpu_matrix_ops_int.h>
...
int cpu_matrix_mult_int(
	...
	if( noisy > 0 ){
		...
		fprintf(stdout, "cpu_matrix_mult_int() : done matrix multiplication of (%zu,%zu) x (%zu,%zu) in %lf seconds.\n", m, n, n, k, time_difference_seconds(TSTARTED, TENDED));
	}
	return 0; // success
}
