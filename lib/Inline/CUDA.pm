...
# holds which subs to import from Inline::C below
# if you are modifying an Inline::C sub, i.e. you copied from Inline::C source
# and pasted/modified in here then you must remove that sub name from this array
# and vice versa, if you no longer modifying a Inline::C sub you need to
# add its name in here (and elminate its implementation from this source code
...
# add the specified subs from Inline::C into our module
# (we would like to inherit from Inline::C really)
# There are some private (my) vars in Inline::C like %fixes used in make_fix()
# but they seem to work fine with what we do below:
# in the future they can become broke
require Inline::C; 
...
# which must be replaced as per the matrix below, '' means replace with nothing=remove them
...
# sub to modify the Makefile in the _Inline/build/* dir
# remove options which gcc does not like and prepend -Xcompiler to others
# so they be passed to gcc via the nvidia cuda compiler (nvcc)
...
# BLIAKO: addition
	my $build_dir = $o->{API}->{build_dir};
	my $flags = Inline::CUDA::Utils::enquire_Makefile_PL($build_dir, 'Makefile.PL', $o->{CONFIG}->{BUILD_NOISY});
...
# BLIAKO: end addition
# BLIAKO: addition (why not ldflagsex????)
...
# BLIAKO: end addition
   ...
   # care should be taken for spaces and quotes!
    ...
    if( ! defined $conf ){ die "call to Inline::CUDA::Utils::read_configuration_file_in_shared_dir() has failed." }

    for my $ak (qw/nvcc nvlink cc cxx ld host_compiler_bindir/){
    	my $AK = uc $ak;
    	if( ! exists($o->{ILSM}{MAKEFILE}{$AK}) || ! defined($o->{ILSM}{MAKEFILE}{$AK}) ){
    		if( ! exists($conf->{$ak}) || ! defined($conf->{$ak}) ){ die "Inline=>CUDA=>Config=>$AK => '...' was not specified in source file and site configuration file does not contain entry '$ak', can not continue" }
    		$o->{ILSM}{MAKEFILE}{$AK} = _quoteme($conf->{$ak});
    	}
    	print STDERR "validate() : found config entry '$AK' => '".$conf->{$ak}."'.\n" if $o->{CONFIG}{BUILD_NOISY};
    }
...
#    }
    ...
    # bliako mod: find the C/inlinecuda dir in share-dir and add it to INC (c's inc that is)
...
C<Inline::CUDA> is not meant to be used directly but rather in this way:
	...
	#    host_compiler_bindir => '/usr/local/gcc84/bin',
...
F<my_cruncher.cu>, whose contents are exactly the same
...
Using C<Inline-E<gt>use(CUDA => "DATA")> seems to have a problem
...
compiled with C<gcc84> (as per the example in section L</INSTALLATION>) with
...
with the special host compiler, e.g. C<gcc84>. The manual on
L<installing Perl|https://metacpan.org/dist/perl/view/INSTALL#C-compiler> states
that specifying the compiler is as simple as C<sh Configure -Dcc=/usr/local/gcc84/bin/gcc84>,
...
From now on, I will be using C</usr/local/gcc84/bin/gcc84> and C</usr/local/gcc84/bin/g++84> as my
host compilers.

=item Now you have our special compiler at C</usr/local/gcc84> under the name C</usr/local/gcc84/bin/gcc84>
and also C</usr/local/gcc84/bin/g++84>. We need to install the CUDA SDK and tell it to skip
...
Notice the cuda program extension C<.cu>. It is important to keep nvcc happy.
...
then you can omit this: C<--compiler-bindir /usr/local/gcc84/bin/gcc84>.
...
But if you need to declare a special host compiler (re: F</usr/local/gcc84/bin/gcc84>)
...
The folder C<demo/> in the base dir of the current distribution
