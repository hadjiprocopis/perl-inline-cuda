...
sub path_of_configuration_file_in_shared_dir {
	...
	if( ! defined $dist_dir ){ print STDERR "$whoami (via $parent) : failed to find dist_dir (where the configuration file must reside), call to File::ShareDir::dist_dir() has failed.\n"; return undef }
...
sub read_configuration_file_in_shared_dir {
	...
	if( ! defined $conf ){ print STDERR "$whoami (via $parent) : call to load_configuration() has failed for file '$conf_file'.\n"; return undef }
	return $conf
}
...
sub test_compilers {
	...
	for my $k (qw/cc cxx ld nvcc host_compiler_bindir/){
		if( ! exists($conf->{$k}) || ! defined($conf->{$k}) ){ print STDERR "$whoami (via $parent) : error, no $k executable has been specified, no key '$k' in configuration hash.\n"; return 1 }
	}
	...
	if( ! defined $tmpfh ){ print STDERR "$whoami (via $parent) : error, call to tempfile() has failed!\n"; return 1 }
	print $tmpfh <<'EOCUDA';
#include <stdio.h>
#define N 10
__global__
