...
sub path_of_configuration_file_in_shared_dir {
	...
	if( ! defined $dist_dir ){ print STDERR "$whoami (via $parent) : failed to find dist_dir (named '$dist_name', where the configuration file must reside), call to File::ShareDir::dist_dir() has failed.\n"; return undef }
