...
use Inline CUDA => Config =>
        ld => '/usr/local/gcc82/bin/g++84',
	#ccflagsex => '-test-bliako-A -test-bliako-B',
	#lddlflags => $Config{lddlflags}.' -test-bliako-C ',
	ccflagsex => '-fPIC',
	lddlflags => $Config{lddlflags}.' -shared',

	clean_after_build => 0,

	# this must fail for version 10.2 and below (check for above)
	#nvccflags => '-forward-unknown-to-host-compiler',
	# this must fail for version 10.2 and below (check for above)
	#nvldflags => '-forward-unknown-to-host-linker',
	...
	# this gets us the Inline object with all options!
	pass_me_o => \$o,
;
...
print "CCFLAGS: ".$o->{ILSM}{MAKEFILE}{CCFLAGS}."\n";
done_testing();
